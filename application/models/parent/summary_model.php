...
class summary_model extends CI_Model {
    ...
    public function GetSummaryDetails($dataObj) {
        ...
        $this->load->model('parent/attendance_model', 'attObj');
    ...
    public function GetStuMarks($dataObj) {
        ...
        $subjectList = $this->coreObj->GetSubjectListFromSection($dataObj['section_id']);
        $arrExamner = array();
        $arrFinal = array();
        for ($j = 0; $j < count($subjectList); $j++) {
            ...
            if (!empty($output)) {
                $staff_id = $output[0]['entry_by'];
                $staff_deatil = $this->coreObj->GetStaffName($staff_id);
