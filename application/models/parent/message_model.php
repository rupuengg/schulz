...
//        exit;
        ...
        foreach ($staffIdList as $row) {
            if ($row > 0) {
                ...
                $staffNameDeatil = $this->coreObj->GetStaffName($row);
                $unreadStatus = $this->GetUnreadMsg($row, $dataObj['adm_no']);
                $totalmessage = $this->GetTotalMsg($row, $dataObj['adm_no']);
                $subjectName = $this->GetSubjectName($row, $dataObj['section_id']);
                $clubName = $this->GetClubName($row);
                $eventName = $this->GetEventName($row);
    ...
    public function GetUnreadMsg($staff_id, $adm_no) {
        $dbObj = $this->load->database($this->session->userdata('database'), TRUE);
    ...
    public function GetTotalMsg($staff_id, $adm_no) {
        $dbObj = $this->load->database($this->session->userdata('database'), TRUE);
    ...
    public function GetClubName($staff_id) {
        $dbObj = $this->load->database($this->session->userdata('database'), TRUE);
        $dbObj->select('name');
    ...
    public function GetEventName($staff_id) {
        $dbObj = $this->load->database($this->session->userdata('database'), TRUE);
        $dbObj->select('name');
    ...
    public function GetSubjectName($staff_id, $section_id) {
        $dbObj = $this->load->database($this->session->userdata('database'), TRUE);
