...
class masterlogin_model extends CI_Model {
    ...
    Public function GetSchoolData($userId) {
        ...
        if (!empty($userdata)) {
            ...
            if ($userdata['member_type'] == 'STAFF') {
                ...
                $staff_details[0]['type'] = $userdata['STAFF'];
                return $staff_details;
            } else if ($userdata['member_type'] == 'PARENT') {
                $student_details = $this->GetStudentDetails($userId, $dynamicDb, $userdata['school_code'], $userdata['usrname']);
    ...
    public function GetStudentDetails($userId, $dynamicDb, $school_code, $username) {
        $dbObj = $this->load->database($dynamicDb, TRUE);
