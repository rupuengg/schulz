...
class cce extends CI_Model {
    ...
    public function GetAllCceSubjectName($staff_id) {
        $dbObj = $this->load->database($this->session->userdata('database'), TRUE);
        $query = "SELECT c.`cce_name`,c.`id` as cce_id,a.`standard`,a.`section`,a.`id` as section_id FROM `cce_staff_relation` as b ,`section_list` as a,`cce_list` as c WHERE  b.`staff_id`='$staff_id' AND c.`id`= b.`cce_id`AND a.`id`=b.`section_id` AND a.`status`='1'";
    ...
    public function GetClassTeacherName($section_id) {
        $dbObj = $this->load->database($this->session->userdata('database'), TRUE);
        $dbObj->select('a.id,a.staff_fname,a.staff_lname');
        ...
        if (!empty($staff_detail)) {
            $staff_name = array('class_teacher' => $staff_detail['staff_fname'] . ' ' . $staff_detail['staff_lname'], 'teachr_id' => $staff_detail['id']);
            return $staff_name;
        } else {
    ...
    public function GetAllCceSection() {
        $dbObj = $this->load->database($this->session->userdata('database'), TRUE);
        ...
        $dbObj->where(array('b.cce_applicable' => 'YES', 'a.status' => '1'));
